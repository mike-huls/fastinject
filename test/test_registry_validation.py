...
def test_can_validate_correct_services_and_configs():
    ...
    registy = Registry(
        services=[services.TimeStamp],
        service_configs=[service_configs.SCDatabase])
...
def test_validate_raises_on_invalid_service_config():
    ...
    # 1. Create registry
    set_default_registry(registry=None)
    registy = Registry(services=[services.TimeStamp], service_configs=[service_configs.SCDatabaseInitFails], auto_validate=False)
    assert registy is not None
    # assert get_default_registry() is not None
...
def test_validate_raises_on_invalid_service():
    ...
    # 1. Create registry
    set_default_registry(registry=None)
    registy = Registry(services=[services.ErrorsOnInit], service_configs=[service_configs.SCDatabase], auto_validate=False)
    assert registy is not None
    # assert get_default_registry() is not None
